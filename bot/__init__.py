...
class RedisPersistence(BasePersistence):
    ...
    def _serialize(self, value: Any) -> str:
        ...
        return self._PICKLE_PREFIX + base64.b64encode(data).decode("ascii")

    def _deserialize(self, data: str) -> Any:
        if data.startswith(self._PICKLE_PREFIX):
            payload = data.removeprefix(self._PICKLE_PREFIX)
    ...
    @staticmethod
    def _conversation_key_to_redis(key: tuple[int | str, ...]) -> str:
        return json.dumps(list(key), separators=(",", ":"))

    @staticmethod
    def _conversation_key_from_redis(value: str) -> tuple[int | str, ...]:
        try:
            ...
            return tuple(int(part) if part.lstrip("-").isdigit() else part
    ...
    async def get_callback_data(self):
        return await self._get(self._key("callback_data"), None)

    async def update_callback_data(self, data):
        if data is None:
            await self._delete(self._key("callback_data"))
            return
    ...
    async def get_conversations(self, name: str) -> dict:
        ...
        for k, v in data.items():
            result[self._conversation_key_from_redis(k)] = v
        return result

    async def update_conversations(self, name: str, data: dict):
        # convert tuple keys to strings for JSON
        store_data = {self._conversation_key_to_redis(k): v
                      for k, v in data.items()}
        await self._set(self._key("conv", name), store_data)

    async def update_conversation(self, name: str, key: tuple, new_state):
